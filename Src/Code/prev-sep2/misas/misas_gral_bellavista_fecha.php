...
<form id="form2" name="form2" method="get" action="<?php echo "misas_gral_bellavista_fecha.php?fechamisa="; ?>">
            ...
            <?
                $list=mysql_query("SELECT DISTINCT (MONTH(Dia)) as Dia FROM sepelio WHERE DATE(dia) <> '0000-00-00' AND Localidad = 3 AND Aprobado = 1 ORDER BY Dia DESC" );
...
<form id="form1" name="form1" method="get" action="<?php echo "misas_gral_bellavista_fecha.php?fechamisa="; ?>">
            ...
            <option value="">--- Seleccione ---</option>
            <?
                $list=mysql_query("SELECT DISTINCT (DATE(Dia)) as Dia FROM sepelio WHERE DATE(dia) <> '0000-00-00' AND MONTH(Dia) = ".$MesMisa." AND Localidad = 3 AND Aprobado = 1  ORDER BY Dia DESC" );
            while($row_list=mysql_fetch_assoc($list)){
                ?>
                    <option value="<? echo $row_list['Dia']; ?>"<? if($row_list['Dia']==$select){ echo "selected"; } ?>>
                                         <?echo $row_list['Dia'];?>
...
$sql= "SELECT Dia, Nombre, HoraMisa,FechaFallecimiento, id FROM sepelio WHERE  Localidad =3 AND LugarMisa=".$iglesias['id']."  AND DATE(Dia) = DATE('".$DiaMisa."') ORDER BY HoraMisa ASC";
...
$sqlotrasiglesias="SELECT distinct(OtroLugar) FROM `sepelio` WHERE Localidad =3 AND LugarMisa = ".$sqliglesia['id']." AND Localidad = 3 AND DATE(Dia) = DATE('".$DiaMisa."')";
