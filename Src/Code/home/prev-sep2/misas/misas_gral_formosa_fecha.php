...
<!--<table width="705" align="center">
  <tr>
    ...
    <div class="container-fluid" align="center">
	 <h1>FORMOSA / MISAS <?php echo $DiaMisa?></h1>
    </div>
<!--   </td>
   </tr>
</table> -->
...
<form id="form2" name="form2" method="get" action="<?php echo "misas_gral_formosa_fecha.php?fechamisa="; ?>">
            Mes :
            <select Name='NEWMES'>
            <option value="">--- Seleccione ---</option>
            <?
                $list=mysql_query("SELECT DISTINCT (MONTH(Dia)) as Dia FROM sepelio WHERE DATE(dia) <> '0000-00-00' AND Localidad = 6 AND Aprobado = 1 ORDER BY Dia DESC" );
        ...
        <br />
 <!--Dropdown -->
<form id="form1" name="form1" method="get" action="<?php echo "misas_gral_formosa_fecha.php?fechamisa="; ?>">
            Fecha :
            <select Name='NEW'>
            <option value="">--- Seleccione ---</option>
            <?
                $list=mysql_query("SELECT DISTINCT (DATE(Dia)) as Dia FROM sepelio WHERE DATE(dia) <> '0000-00-00' AND MONTH(Dia) = ".$MesMisa." AND Localidad = 6 AND Aprobado = 1  ORDER BY Dia DESC" );
            while($row_list=mysql_fetch_assoc($list)){
                ?>
                    <option value="<? echo $row_list['Dia']; ?>"<? if($row_list['Dia']==$select){ echo "selected"; } ?>>
                                         <?echo $row_list['Dia'];?>
...
$sql= "SELECT Dia, Nombre, HoraMisa,FechaFallecimiento, id FROM sepelio WHERE Localidad =6 AND LugarMisa=".$iglesias['id']."  AND DATE(Dia) = DATE('".$DiaMisa."') ORDER BY HoraMisa ASC";
...
$sqlotrasiglesias="SELECT distinct(OtroLugar) FROM `sepelio` WHERE Localidad =6 AND LugarMisa = ".$sqliglesia['id']." AND Localidad=6 AND DATE(Dia) = DATE('".$DiaMisa."')";
