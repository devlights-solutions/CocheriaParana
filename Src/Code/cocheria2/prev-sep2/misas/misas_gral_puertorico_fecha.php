...
$sqliglesia ="SELECT * FROM  `iglesias` WHERE AND Nombre =  'Otro'";
$sqliglesia = mysql_query($sqliglesia);
$sqliglesia = mysql_fetch_array($sqliglesia);

$sqlotrasiglesias="SELECT distinct(OtroLugar) FROM `sepelio` WHERE  Localidad =8  LugarMisa = ".$sqliglesia['id']." AND Localidad=8 AND DATE(Dia) = DATE('".$DiaMisa."')";
