...
class nusoap_server extends nusoap_base {
	...
	*/
	function invoke_method() {
		...
		if (strpos($this->methodname, $deliStrStat) > 0) {
			$delim = $deliStrStat;
		} else if (strpos($this->methodname, $deliStrCls) > 0) {
			$delim = $deliStrCls;
		} else {
			$delim = '';
		}
		...
		if (!function_exists('call_user_func_array')) {
			...
			if ($class == '') {
				...
				if(defined('_JEXEC')){
					//we are indeed in the Joomla Framework, so the function in the plugin can be called
					$dispatcher =& JDispatcher::getInstance();
					$instance = new $class ($dispatcher);
				}else{
					$instance = new $class ();
				}
				//if(class_exists($class)) $dispatcher = JDispatcher::getInstance();
    ...
    */
    function webDescription(){
		...
		<div class=content>
			...
			<div class=nav>
				<p>View the <a href="' . $linkWSDL . '">WSDL</a> for the service.
