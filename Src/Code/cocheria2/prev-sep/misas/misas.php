...
<tr><td id="titulo_misas">Próximas Misas:</td></tr>
<?php
$sql= "SELECT distinct cod_mis, lugar_mis, fecha_mis, dia_mis FROM misas WHERE localidad_mis='".$salloc."' AND fecha_mis >= CURRENT_DATE() ORDER BY fecha_mis ASC";
...
while($mis = mysql_fetch_array($res))
{
		...
		echo "
			<tr><td id='desc_misas'><a href='misas_detalle.php?femis=".$mis['fecha_mis']."&salsep=".$_GET['salsep']."'>".ucwords(strtolower($mis['dia_mis']))." - ".$partes[3]."/".$partes[2]."/".$partes[1]."-".ucwords(strtolower($mis['lugar_mis']))."</a></td></tr>
		";
